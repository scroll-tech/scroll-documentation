...
whatsnext: { "Rollup 节点": "/technology/sequencer/rollup-node/" }
---
...
- **状态和存储树**：以太坊使用 MPT (Merkle-Partricia Trie) 作为其状态和合约存储的数据结构。该树的结构以及其使用的 Keccak 哈希使得 ZK 电路变得非常昂贵。 而 `l2geth` 使用 [zkTrie](/technology/sequencer/zktrie)：一种使用 Poseidon 哈希默克尔二叉树，来存储状态和合约数据。
