...
:::tip[Rollup Status]
The rollup status `Finalized` indicates that the correct execution of transactions in this block has been proven by verifying a validity proof on-chain on Sepolia.
{/* For more information about rollup status, see [Scroll's Architecture Overview](/technology/architecture/scroll-architecture). */}
:::
