...
This document describes the rollup process in Scroll. The figure below illustrates the rollup workflow.
...
The rollup node contains two modules:
...
- **Chunk Proposer** and **Batch Proposer** proposes new chunks and new batches following the constraints described in the [Transaction Batching](/RxRWLFtiQ7W_wbc0jMnQNA).
...
5. If the criterion (described in the [Transaction Batching](/RxRWLFtiQ7W_wbc0jMnQNA)) are met, the rollup node proposes a new [chunk](/54Rp7dBOQIqAEWtmGT8_rQ#Chunk) or a [batch](/54Rp7dBOQIqAEWtmGT8_rQ#Batch) and writes it to the database. Otherwise, the rollup node keeps waiting for more blocks or chunks.
