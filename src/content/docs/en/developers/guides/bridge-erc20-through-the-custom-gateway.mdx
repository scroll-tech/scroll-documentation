...
excerpt: "This guide will walk through how to use Scroll's bridge for ERC20 tokens that need custom functionality by using the Custom Gateway."
---
...
contract L2Token is ERC20, IScrollERC20Extension {
  // We store the gateway and the L1 token address to provide the gateway() and counterpart() functions needed from the Scroll Standard ERC20 interface
  address _gateway;
