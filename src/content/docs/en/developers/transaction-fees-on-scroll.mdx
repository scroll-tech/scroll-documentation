...
All tx fees are collected into the `L2ScrollFeeVault` contract, predeployed at address `0x5300000000000000000000000000000000000005`. The block producer receives no direct reward. (The `COINBASE` opcode returns the fee vault address.) This contract also tracks the amount we’ve historically withdrawn to L1.

To check the amount of ETH withdrawn to L1 using [Foundry Cast](https://book.getfoundry.sh/cast/):

```bash
cast call --rpc-url "https://sepolia-rpc.scroll.io" "0x5300000000000000000000000000000000000005" "totalProcessed()(uint256)" | cast --from-wei
```

To check the fees that have been collected but not yet withdrawn to L1:

```bash
cast balance --rpc-url "https://sepolia-rpc.scroll.io" "0x5300000000000000000000000000000000000005" | cast --from-wei
```
