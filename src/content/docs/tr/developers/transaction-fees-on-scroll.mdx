...
section: developers
date: Last Modified
title: "Scroll'de İşlem Ücretleri"
lang: "tr"
...
Scroll'de işlem ücretleri, destek verdiği katmana(Katman 1) göre oldukça düşüktür. Kullanıcılar ve geliştiriciler için Scroll'deki işlem ücretleri, Ethereum ana ağında olduğuna benzer şekilde çalışıyor gibidir. Mevcut araçlar, cüzdanlar ve kodlar da büyük ihtimalle oldukları gibi çalışacaklardır. Ancak cüzdanda gösterilen işlem ücreti, kullanılan yazılım Scroll'un ücret hesaplamalarından özellikle haberdar olmadığı sürece resmin tamamını içermeyecektir.

L2 rollup'larındaki işlem maliyetleri tasarımları nedeniyle L1'in maliyetlerine bağlıdır. Ethereum'un güvenliğinden yararlanmak için Scroll'ün, L1'de veri saklanması ve doğrulanmasında gereken işlem verilerinin ve kanıtların maliyetini de hesaba katması gerekir.
...
- **L1 ücreti**
  ...
  - ETH, ücret karşılığında kullanıcının Scroll'deki bakiyesinden otomatik olarak düşülür

Yüksek düzeyde, **L2 ücreti** işleminizi L2 sıralayıcısında gerçekleştirmenin maliyetidir ve **L1 ücreti** ise bu işlemi L1'de gerçekleştirmenin maliyetidir.
...
Blok üreticisi doğrudan bir ödül almaz ve 'COINBASE' işlem kodu ücret kasası adresini(fee vault address) döndürür.
...
Scroll, önceden konuşlandırılmış bir "L1GasPriceOracle"a sahiptir ve hem Scroll hem de Scroll Sepolia'da ([`0x5300000000000000000000000000000000000000002`](https://sepolia-blockscout.scroll.io/address/0x53000000000000000000000) adresinden erişilebilir 0000000000000000002). Belirli bir işlemin ham verileri için L1 veri ücretini tahmin etmek için bir "getL1Fee" yöntemi sağlar.
...
- İmzalı RLP kodlu bir işlemin "sıfırları" ve "sıfır olmayan" baytları
...
2. İşlemdeki sıfır baytları ve sıfır olmayan baytları say.
3. `TX_DATA_ZERO_GAS = 4`, `TX_DATA_NON_ZERO_GAS = 16`[^eip-2028] ve `PRECISION = 1e9`'i göz önüne alarak L1 veri ücretini hesapla:
   ...
   RLP kodlu işlemdeki bayt sayısını depolamak için sıfır olmayan baytlarda ek 4 bayt ayırırız.
...
Bir işlem için kullanılan L1 gazının miktarını hesaplar. İşlemin ve durum köklerinin L1'e kaydedilmesine ilişkin işlem başına gaz ek yükünü temsil eden ek yükü ekler. Giriş işleminin imzası olmadığı gerçeğini hesaba katarak 74 baytlık dolgu ekler.
...
L1'den L2'ye mesajlaşırken kullanıcı L1'deki tüm işlem ücretlerini öder. Kullanıcı L1'in kendi gaz ücretini öder ancak bu sebepten Scroll'e L1 Veri Ücreti ödemesine gerek yoktur. Fakat L1 işlemlerinde L2 Yürütme Ücretlerinin hesaba katılması ve ne kadar L2 gazı ödeyeceklerini bilmeleri gerekecek.
...
Şu anda, L1'de kanıt üretimi ve kanıt doğrulaması için gaz için gereken hesaplama, Scroll ve çeşitli kanıtlama ortakları tarafından desteklenmektedir. L2 Gaz Fiyatına bir taban değer belirlenerek kısmen halledilmiştir.
