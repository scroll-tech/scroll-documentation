...
Scroll ile akıllı sözleşmeler oluşturmak ve test ederken en sevdiğiniz araçlar oldukları gibi çalışırlar.

Scroll, EVM ile bayt kodu olarak denk olduğundan, favori geliştirici araçlarınızı bir Scroll RPC Sağlayıcısına yönlendirmeniz yeterlidir.
...
Scroll üzerinde inşa etmeye başlamak için Scroll Sepolia test ağımızı kullanarak başlamanızı öneririz. Öncelikle bir miktar test ağı ETH edinmeniz gerekecek. Sepolia'da test token'ları almayla ilgili ipuçları için [Musluk](/tr/user-guide/faucet) sayfasına bakın. Bundan sonra, [Köprü makalesinde](/tr/user-guide/bridge) açıklandığı gibi [Sepolia Köprümüzü](https://sepolia.scroll.io/bridge) kullanarak test ağı ETH'lerinizi Scroll Sepolia Testnet'e (Katman 2) aktarabilirsiniz.

Adım adım açıklama için Kullanım Kılavuzunun [Kurulum](/tr/user-guide/setup) sayfasıyla başlayın.

Scroll ana ağında akıllı sözleşme dağıtımak için hazır olduğunuzda, [yerel köprümüzü](https://scroll.io/bridge/) veya 3. taraf köprülerden birini kullanarak ETH üzerinden köprü kurabilirsiniz.
...
Hardhat yapılandırma dosyanızı `hardhat.config.ts'yi Scroll Sepolia Testnet public RPC'sini işaret edecek şekilde değiştirin.
...
Scroll Sepolia Testnet Public RPC'yi kullanarak dağıtım yapmak için şunu çalıştırın:
...
<Aside type="danger" title="warning">
  Bazı proje çatallarında Etherscan tarafından sağlanan sözleşme doğrulamasını devre dışı bırakmak isteyebilirsiniz. Bu, "packages/hardhat/deploy/00_deploy_your_contract.js" dosyasında yorumlanabilir.
</Aside>
