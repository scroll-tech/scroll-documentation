...
Sıradaki adımda, Sepolia'daki orijinal tokenı temsil edecek olan bir karşılık tokenını Scroll üzerinde başlatacaksınız. Bu token, L1 tokenininkiyle eşleşmek için özel mantık uygulayabilir veya hatta L1 tokeninin ötesinde ek özellikler ekleyebilir.
...
Tokenları Scroll'e ve Scroll'den köprülemek için önerilen yöntem (yukarıda açıklandığı gibi) tokenınızı Scroll resmi köprüsüne eklemektir. Bu yaklaşım, tokenınızın keşfedilmesini kolaylaştıracak ve sahipleri için daha güvenli hale getirecektir. Ancak bunun için Scroll ekibinin onayı gereklidir. Resmi onay süreci olmadan özel bir token başlatmak istiyorsanız kendiniz özel bir ağ geçidi oluşturabilirsiniz. Bunu yapmak için L1'de bir "L1CustomERC20Gateway" sözleşmesi ve L2'de bir "L2CustomERC20Gateway" dağıtmanız gerekecektir.
...
Şimdi Scroll'de muadil sözleşmeyi başlatalım.
