...
section: technology
date: Last Modified
title: "Yatırım Ağ Geçitleri (Deposit Gateways)"
lang: "tr"
...
| `L1CustomERC20Gateway`   | Özel (custom) ERC20 token yatırma işlemleri için ağ geçidi                   |
...
1. 'L1GatewayRouter', ETH'yi L1'den L2'ye yatırmak için üç fonksiyon sağlar. 'depositETHAndCall' fonksiyonu, aynı anda ETH'i aktarabilir ve bir kontrat çağrısı (contract call) gerçekleştirebilir.
...
6. İlgili L2 işlemi, L2'deki para yatırma işlemini tamamlamak için 'L2ScrollMessenger.relayMessage' fonksiyonunu çağırır. Standart ERC20 token yatırma durumunda, işlem daha sonra 'L2StandardERC20Gateway.finalizeDepositERC20'yi çağırır.
7. Bu ERC20 token sözleşmesi L2'de konuşlandırılmamışsa, 'L2StandardERC20Gateway' mesajdan token meta verilerini çıkaracak ve standart ERC20 tokenını L2'de dağıtmak için 'ScrollStandardERC20Factory'yi çağıracaktır.
...
Büyük miktarda ERC-721 veya ERC-1155 token yatırma işlemini kolaylaştırmak için, aşağıdaki işlevler aracılığıyla 'L1ERC721Gateway' ve 'L1ERC1155Gateway' sözleşmesinde toplu para yatırma işlevleri de sağlıyoruz:
