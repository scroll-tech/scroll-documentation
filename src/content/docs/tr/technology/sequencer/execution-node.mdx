...
- Blokları yürütür (execute eder) ve L2 zinciri sürdürür.
...
- Eşlerin (takipçi düğümlerinin) Ethereum'un eşler arası protokolünü kullanarak blok zincirini senkronize etmesine olanak sağlar.

[go-ethereum](https://github.com/ethereum/go-ethereum)'un bir çatalı olan yürütme düğümü, işlevlerinin çoğunu Ethereum'dan devralır. Buna işlem ve blok veri yapıları, EVM yürütme, RPC ve p2p protokolleri dahildir. Bu veri yapıları ve protokoller doğrudan "go-ethereum"dan miras alındığı için, Scroll'un Ethereum ile uyumluluğu konusunda çok yüksek bir kesinliğe sahip olabiliriz.
...
[`l2geth`](https://github.com/scroll-tech/go-ethereum), Scroll'un [go-ethereum](https://github.com/ethereum/go-ethereum) çatalıdır. Blokların oluşturulmasından, yürütülmesinden ve blockchain durumunun sürdürülmesinden sorumludur. 'l2geth', aşağıda listelenen bazı önemli farklılıklarla birlikte, işlevlerinin çoğunu Ethereum'dan devralmaktadır. 'l2geth' aşağıdaki alt modüllere sahiptir (kapsamlı liste değildir):
...
- **API layer**: Standart Ethereum RPC ve p2p arayüzleri.
- **Transaction pool**: L2 işlemleri için bellek havuzu.
...
Bu, asla kanıtlanamaz parçalar önermediğinden emin olmak için devre kapasitesi denetleyicisinin toplama rölesine dahil edilmesiyle elde edilir.
...
- **State and storage tree**: Ethereum, durum ve sözleşme depolama veri yapısı olarak MPT'yi (Merkle-Partricia Trie) kullanır. Bu trie'nin yapısı ve Keccak karma değerini kullanması, onu ZK devreleri için aşırı derecede pahalı hale getirecektir. 'l2geth' bunun yerine [zkTrie](/tr/technology/sequencer/zktrie) kullanır: durumu ve sözleşme depolaması için Poseidon karma değerine sahip ikili bir Merkle trie'si.
